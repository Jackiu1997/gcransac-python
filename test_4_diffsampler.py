...
if __name__ == "__main__":
    ...
    threshold = 1.0
    match_img_list = []
    sampler_sets = ["Uniform", "PROSAC", "P-NAPSAC"]
    H, mask = None, None
    for i in range(len(sampler_sets)):
        print(sampler_sets[i])
        if i == 0:
            H, mask = gc.findHomography(src_pts, dst_pts, h1, w1, h2, w2, threshold=threshold, conf=0.99, max_iters=10000, sampler=sampler_sets[i])
        elif i == 1:
