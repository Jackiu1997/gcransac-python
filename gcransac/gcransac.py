...
class GCRANSAC:
	...
	def run(self,
		...
		# init H(|L∗|, µ)
		max_iteration = self.__getIterationNumber(1)

		# for k = 1 →H(|L∗|, µ) do
		while self.statistics.iteration_number < min(max_iteration, self.settings.max_iteration_number) or\
                        self.settings.min_iteration_number > self.statistics.iteration_number:
			do_local_optimization = False
			# 增加迭代计算次数
			self.statistics.iteration_number += 1

			# Sk ← Draw a minimal sample
			sample = self.__sample(pool, self.sample_number)
			# 检查采样是否有效，无效则重新评估
			if len(sample) == 0 or\
			...
			for model in models:
				...
				if so_far_the_best_score < score:
					...
					# 更新最大迭代数
					max_iteration = self.__getIterationNumber(so_far_the_best_score.inlier_number)
	...
	def __sample(self, pool, sample_number, local_optimization=False):
		""" 从给定的采样池和采样大小确定样本
		...
		if (local_optimization):
			return self.local_optimization_sampler.sample(pool, sample_number)
		return self.main_sampler.sample(pool, sample_number)

	# H(|L∗|, µ)
	def __getIterationNumber(self, inlier_number):
		""" 计算当前内点数目期望的迭代数目 """
		sample_size = self.estimator.sampleSize()
		q = (inlier_number / self.point_number) ** sample_size
		log2 = m.log(1 - q)
		log_probability = m.log(1.0 - self.settings.confidence)

		if abs(log2) < sys.float_info.epsilon:
			return sys.maxsize
		return int(log_probability / log2)+ 1

		'''
		Pi = (inlier_number_ / self.point_number) ** self.estimator.sampleSize()
		log1 = m.log(self.settings.confidence)
		log2 = m.log(1.0 - Pi)
		return int(log1 / log2) + 1
		'''

	def __getConfidenceNumber(self, inlier_number_, sample_size_, confidence_):
		# k = log(1 − µ)/ log(1 − η^m)
		inlier_ratio = float(inlier_number_) / self.point_number
		k = m.log(1 - confidence_) / m.log(1 - inlier_ratio ** sample_size_)
		# µ = 1 − 10^k*log(1−η^m)
		confidenceNumber = 1 - 10 ** (k * m.log(1 - inlier_ratio ** sample_size_))
	...
	def __localOptimization(self,
		...
		changed = False
		graph_cut_number = 0
		while not changed and graph_cut_number < self.settings.max_graph_cut_number:
			graph_cut_number += 1
			...
			if sample_size < len(gc_inliers):
				current_sample = self.__sample(gc_inliers,
											   sample_size,
